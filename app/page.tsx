...
function InventoryPage() {
  ...
  const exportToCSV = useCallback(async () => {
    const headers = ['ID', 'Name', 'Quantity', 'Cupboard', 'Category', 'Serial Number', 'Model Number', 'IMEI', 'Adapter', 'Cable', 'Sim', 'Box', 'Remark', 'Working'];
      ...
      try {
        ...
        wb.SheetNames.forEach(wsname => {
          ...
          const parsedItems: InventoryItem[] = data.map((row, index) => {
            ...
            const cupboard = String(row.Drawer || row.cupboard || row.Cupboard || '1').trim();
  ...
  const requestSuggestions = useMemo(() => {
    if (!requestItemName || requestModal?.item) return [];
    return data.items
      .filter(item => item.name.toLowerCase().includes(requestItemName.toLowerCase()))
      .slice(0, 5);
  }, [data.items, requestItemName, requestModal?.item]);
              ...
              {requestModal.item ? (
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button 
                      onClick={() => setRequestModal({ ...requestModal, item: undefined })}
                      className="text-[9px] font-bold uppercase text-blue-600 hover:text-blue-700"
                    >
                      Change
                    </button>
                  </div>
                  ...
                  {requestSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-100 rounded-xl shadow-xl z-50 overflow-hidden">
                      <div className="p-2 border-b border-gray-50 bg-gray-50/50">
                        <p className="text-[9px] font-bold uppercase text-gray-400 tracking-widest">Available in Inventory</p>
                      </div>
                      {requestSuggestions.map(item => (
                        <button
                          key={item.id}
                          onClick={() => {
                            setRequestModal({ ...requestModal, item });
                            setRequestItemName('');
                          }}
                          className="w-full text-left p-3 hover:bg-blue-50 transition-colors flex items-center justify-between group"
                        >
                          <div className="flex flex-col">
                            <span className="text-xs font-bold text-gray-900 group-hover:text-blue-600">{item.name}</span>
                            <span className="text-[10px] text-gray-400">Loc: {item.cupboard.length <= 2 ? `C${item.cupboard}` : item.cupboard}</span>
                          </div>
