      ...
      return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-red-50">
          <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-red-100">
            ...
            <pre className="text-xs bg-gray-50 p-4 rounded-lg overflow-auto mb-4 text-red-600">
...
function InventoryApp() {
  ...
  const [exports, setExports] = useState<{ name: string; url: string }[]>([]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      ...
      setIsAuthReady(true);
      if (u) {
        // Admin check: email or role in Firestore
        const isAdminEmail = u.email === 'pradnya@mintlabs.in';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, 'inventory'), (snapshot) => {
      ...
      setStatus('Inventory Synchronized');
    }, (error) => {
      handleFirestoreError(error, 'list', 'inventory');
    });

    return () => unsubscribe();
  }, [isAuthReady, user, handleFirestoreError]);

  // Fetch Exports from Storage
  const fetchExports = useCallback(async () => {
    if (!user) return;
    try {
      const listRef = ref(storage, 'exports');
      ...
      console.error('Failed to fetch exports from storage:', err);
    }
  }, [user]);
  ...
  const updateLastAction = async (action: string) => {
    try {
      ...
      setData(prev => ({ ...prev, lastAction: action }));
    } catch (error) {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      ...
      handleFirestoreError(error, 'create', `inventory/${id}`);
    }
  };
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          ...
          handleFirestoreError(error, 'write', 'inventory (batch)');
        }
      }
    });
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      handleFirestoreError(error, 'update', `inventory/${editForm.id}`);
    }
  };

  const exportToCSV = async () => {
    const headers = ['ID', 'Name', 'Quantity', 'Cupboard', 'Shelf', 'Category', 'Serial Number', 'Model Number', 'Last Updated', 'Updated By'];
    ...
    try {
      ...
      setStatus(`Inventory exported and saved to cloud: ${fileName}`);
      await fetchExports();
    } catch (error) {
      console.error('Cloud export failed:', error);
      setStatus('Exported to browser, but cloud save failed');
    }
  };
      ...
      <div className="min-h-screen bg-[#F5F5F4] flex items-center justify-center p-4 font-sans">
        ...
        >
          ...
          <div className="space-y-6">
            <p className="text-center text-sm text-gray-600">Please sign in to access the inventory system.</p>
            <button 
              onClick={handleLogin}
              className="w-full bg-white border border-gray-200 text-gray-700 p-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-gray-50 transition-all shadow-md active:scale-[0.98]"
            >
              ...
              {isAdmin && (
                ...
                <div className="pt-6 border-t border-gray-100">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Recent Cloud Exports</h3>
                  </div>
